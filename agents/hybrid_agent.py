...
class StudentAgent(Agent):
    ...
    class Node:
        ...
        def expand(self):
            ...
            child = StudentAgent.Node(new_board, next_player, self.outer, move, self)
    ...
    def mcts(self, root_board, player, time_limit=1.5):
        ...
        while time.time() - start < time_limit:
            ...
            if node.untried_moves:
                ...
                if child:
                    node = child
            result = self.simulate(node.board.copy(), node.player, root_player)
    ...
    def step(self, board, player, opponent):
        ...
        #return self.minimax_mcts_step(board, player, opponent, depth=3, mcts_budget=0.4)
        # 1-run MCTS at root
        root = StudentAgent.Node(board.copy(), player, self, move=None, parent=None)

        self.run_root_mcts(root, time_limit=0.4)
        ...
        # do hybrid selection using a *single* MCTS tree
